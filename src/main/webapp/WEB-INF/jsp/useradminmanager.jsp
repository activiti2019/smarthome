...
<body>
		...
		<div id="page-wrapper">
			...
			<div class="panel panel-default" style="min-width: 600px;">
				<div class="panel-body">
					<form class="form-inline" name="formSubmit"
						onsubmit="return false;"
						action="${pageContext.request.contextPath }/user2/list.action"
						method="post">
				...
				<div class="col-lg-12">
					<div class="panel panel-default" style="min-width: 600px;">
						...
						<div class="col-md-12 text-right">
							<p:page
								url="${pageContext.request.contextPath }/user2/list.action" />
						</div>
						<!-- /.panel-body -->
					</div>
					<!-- /.panel -->
	...
	<script type="text/javascript">
		...
		function editUser(uId) {
			$.ajax({
				type:"get",
				url:"<%=basePath%>user2/edit.action",
				data:{"uId":uId},
		...
		function updateUser() {
			...
			$.post("<%=basePath%>user2/update.action",$("#edit_user_form").serialize(),function(data){
				alert("用户信息更新成功！");
		...
		function deleteUser(uId) {
			if(confirm('是否确认禁用该用户?')) {
				$.post("<%=basePath%>user2/delete.action",{"uId":uId},function(data){
					alert("禁用成功！");
					window.location.reload();
				});
			}
		...
		function restoreUser(uId) {
			if(confirm('是否确认恢复该用户?')) {
				$.post("<%=basePath%>user2/restore.action",{"uId":uId},function(data){
					alert("恢复成功！");
					window.location.reload();
				});
			}
		...
		function search(){
			...
			if(""!=pagenum){
				document.formSubmit.action="${pageContext.request.contextPath }/user2/list.action?page="+pagenum+"&rows=10";
			}
