...
<body>
		...
		<div id="page-wrapper">
			...
			<form style="float:left;margin-left: 200px;" id="formselect">
				<input type="number"  name="selectYear" value="2017" style="width: 100px;" id="selectyear" oninput="if(value.length>4)value=value.slice(0,4)"/>
				<button type="button" onclick="search();">查询</button>
				<span id="yearerror" style="color: gray;">请输入表示年份四位数字，不能以0开始</span>
			</form>
		<br />
		<br />
...
<!-- 			</div> -->
	...
	$(document).ready(function() {
		...
		var errorsum = [];
		var alldate = [];
		var selectYear = $("input[name='selectYear']").val();
		$.ajax({
			type : "post",
			async : true,
			url : "../record/selectByMonthEveryDayMonth",
	...
	function search()  {
		...
		var patt20 = new RegExp("^[1-9][0-9]{3}$");
		if(!( patt20.test(selectyear))){
			document.getElementById('yearerror').style.color='red';
			$('#yearerror').text('年份格式错误');
		}else{
			$('#yearerror').empty();
		var mychart = echarts.init(document.getElementById("main"));
		option = {
